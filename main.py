...
class ImageCaptionDataset(Dataset):
    def __init__(self, image_dir, captions_file, tokenizer, max_length,
        ...
        efficientnet_transform = weights.transforms()
...
class TransformerDecoder(nn.Module):
    def __init__(self, embed_dim, num_heads, hidden_dim, vocab_size, num_layers, max_length, feature_dim, dropout, num_image_tokens):
        ...
        # Load frozen BART embeddings
        bart_model = BartModel.from_pretrained("facebook/bart-base")
        self.embedding = bart_model.get_input_embeddings()
        self.embed_dim = self.embedding.embedding_dim
        self.embedding.weight.requires_grad = False  # Freeze the weights
        
        # Dropout
        self.dropout = nn.Dropout(dropout)
        
        # Positional encoding
        self.register_buffer('positional_encoding', self._generate_positional_encoding(max_length, self.embed_dim))
        
        # Project encoder's spatial features to decoder embedding space.
        self.feature_proj = nn.Sequential(
            nn.Linear(feature_dim, self.embed_dim),
            nn.LayerNorm(self.embed_dim)  # Normalize features for stability
        )
        
        # Learnable positional embeddings for image tokens.
        self.image_pos_embedding = nn.Parameter(torch.randn(1, num_image_tokens, self.embed_dim))
        
        # Extra Transformer encoder block for image features
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=num_heads, dim_feedforward=hidden_dim, dropout=dropout, batch_first=True)
        self.image_feature_encoder = nn.TransformerEncoder(encoder_layer, num_layers=1)
        
        # Transformer decoder layers.
        decoder_layer = nn.TransformerDecoderLayer(d_model=self.embed_dim, nhead=num_heads, dim_feedforward=hidden_dim, dropout=dropout, batch_first=True)
...
class ImageCaptionTrainer:
    ...
    def evaluate_test_set(self, test_loader, max_length):
        ...
        return all_hypotheses, all_references, bleu_hypotheses, bleu_references

    def compute_metrics(self, all_hypotheses, all_references, bleu_hypotheses, bleu_references):
        # Compute BLEU
...
def generate_caption(image_path, model, model_name, tokenizer, device, max_length):
    image = process_image(image_path, model_name)
...
def inference_efficientnet():
    ...
    while True:
        ...
        caption = generate_caption(IMAGE_PATH, model,"efficientnet", tokenizer, device, MAX_LENGTH)
...
def inference_deit():
    ...
    while True:
        IMAGE_PATH = input("Enter image path: ")
        caption = generate_caption(IMAGE_PATH, model, "deit", tokenizer, device, MAX_LENGTH)
        print("Generated Caption (beam):", caption)

def inference():
    while True:
        try:
            model_id = input("Enter model name (1: efficientnet, 2: deit): ").strip().lower()
